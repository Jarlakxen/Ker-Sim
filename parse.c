/* cpu parser - parses source and executes statements */
...
struct Value *ParseFunctionDefinition(struct ParseState *Parser, struct ValueType *ReturnType, char *Identifier) {
	...
	FuncValue = VariableAllocValueAndData(Parser, sizeof(struct FuncDef) + sizeof(struct ValueType *) * ParamCount + sizeof(const char *) * ParamCount, FALSE,
											NULL, TRUE);
	...
	if (strcmp(Identifier, "main") == 0) {
		...
		if (FuncValue->Val->FuncDef.ReturnType != &IntType && FuncValue->Val->FuncDef.ReturnType != &VoidType)
			ProgramFail(Parser, "main() should return an int or void");

		if (FuncValue->Val->FuncDef.NumParams != 0 && (FuncValue->Val->FuncDef.NumParams != 2 || FuncValue->Val->FuncDef.ParamType[0] != &IntType))
...
void ParseDeclarationAssignment(struct ParseState *Parser, struct Value *NewVariable, int DoAssignment) {
	...
	if (LexGetToken(Parser, NULL, FALSE) == TokenLeftBrace) {
		...
		for (ArrayIndex = 0; (Parser->Mode != RunModeRun && Token == TokenComma) || (Parser->Mode == RunModeRun && ArrayIndex < NewVariable->Typ->ArraySize);
				ArrayIndex++) {
			...
			if (Parser->Mode == RunModeRun)
				ArrayElement = VariableAllocValueFromExistingData(
						...
						(union AnyValue *) (&NewVariable->Val->ArrayMem[0] + TypeSize(NewVariable->Typ->FromType, 0, TRUE) * ArrayIndex), TRUE, NewVariable);
